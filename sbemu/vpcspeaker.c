...
//
// Author: andersrodrig
// Project: https://github.com/andersrodrig/sbemu
//
// SBEMU integration:
...
//   - Mixes the generated audio into the SBEMU output stream.
...
#include <string.h>
#include <pc.h>
#include <untrapio.h>   // UntrappedIO_OUT_Handler / UntrappedIO_IN_Handler

#if SBEMU_PCSPEAKER

#define PIT_FREQ 1193182 // 1.193182 MHz
#define SPKR_VOLUME 10000 // Master volume for PC Speaker (0-32767 range)
...
void VPCSPEAKER_Init(int output_freq)
{
    ...
    // Read real port 61 to sync state (via untrapped handler to avoid recursion)
    vpcspeaker_set_port61(UntrappedIO_IN_Handler(0x61));
...
// ---------------------------------------------------------------------------
// IO Handler (matches QEMM_IODT handler signature)
// ---------------------------------------------------------------------------
uint32_t VPCSPEAKER_IOHandler(uint32_t port, uint32_t val, uint32_t out)
{
    ...
    if(out) // Write
    {
        switch(port)
        {
            ...
            {
                UntrappedIO_OUT_Handler(0x43, val); // passthrough (bypass trap)
            ...
            {
                UntrappedIO_OUT_Handler(0x42, val); // passthrough (bypass trap)
            ...
            {
                UntrappedIO_OUT_Handler(0x61, val); // passthrough (bypass trap)
    ...
    else // Read
    {
        ...
        {
            case 0x43:
                return UntrappedIO_IN_Handler(0x43); // passthrough
            case 0x42:
                // Return real PIT value to avoid breaking DOS timing routines
                return UntrappedIO_IN_Handler(0x42);
            case 0x61:
            {
                ...
                uint8_t real61 = UntrappedIO_IN_Handler(0x61);
