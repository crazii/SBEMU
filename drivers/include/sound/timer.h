...
static inline int snd_timer_new (struct snd_card *card, char *id, struct snd_timer_id *tid, struct snd_timer **rtimer) {
  struct snd_timer *tmr = kzalloc(sizeof(*tmr), GFP_KERNEL);
  if (!tmr) {
    return -ENOMEM;
  }
  *rtimer = tmr;
