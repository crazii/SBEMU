...
int ct_pcm_playback_open(struct snd_pcm_substream *substream)
{
...
int ct_pcm_playback_prepare(struct snd_pcm_substream *substream)
{
	int err;
	struct ct_atc *atc = snd_pcm_substream_chip(substream);
	struct snd_pcm_runtime *runtime = substream->runtime;
...
ct_pcm_playback_pointer(struct snd_pcm_substream *substream)
{
  ...
  position = atc->pcm_playback_position(atc, apcm);
	position = bytes_to_frames(runtime, position);
        //printk("%s:%d pos %u bs %u\n",__FILE__,__LINE__,position,runtime->buffer_size);
...
const struct snd_pcm_ops ct_pcm_playback_ops = {
