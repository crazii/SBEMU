...
//#define IEC958_DEFAULT_CON (0x8204 | (0x10<<16) | (1<<25))
...
static const struct snd_pci_quirk subsys_20k2_list[] = {
	...
	SND_PCI_QUIRK(PCI_VENDOR_ID_CREATIVE, PCI_SUBDEVICE_ID_CREATIVE_SB08803,
		      "SB0880", CTSB0880),
	SND_PCI_QUIRK(PCI_VENDOR_ID_CREATIVE, PCI_SUBDEVICE_ID_CREATIVE_SB08803+1, // 0x44
		      "SB0880", CTSB0880),
...
{
  //printk("atc_pcm_playback_prepare\n");
  //printk("apcm: %8.8X\n", apcm);
  //printk("apcm->substream: %8.8X\n", apcm->substream);
  //printk("apcm->substream->runtime: %8.8X\n", apcm->substream->runtime);
  //printk("apcm->substream->pcm: %8.8X\n", apcm->substream->pcm);
	...
	if (err < 0)
        ...
        //printk("atc_pcm_playback_prepare 2\n");
	/* Connect resources */
	src = apcm->src;
...
atc_pcm_playback_position(struct ct_atc *atc, struct ct_atc_pcm *apcm)
{
	...
	int ret = (position + size - max_cisz - apcm->vm_block->addr) % size;
        //if ((xfi_int_cnt % 500) == 0) printk("bufpos %u\n", ret);
...
int ct_atc_destroy(struct ct_atc *atc)
{
	int i = 0;
